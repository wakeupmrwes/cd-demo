...
EOF
            ...
            stage("${variant}:${channel} Verify demo success") {
              timeout(30) {
                waitUntil {
                  def response = sh(
                    returnStatus: true,
                    script: "http --check-status --ignore-stdin --timeout=2.5 HEAD 'http://${elb_url}/' &> /dev/null"
                  ).trim()
        ...
        // Clean-up DC/OS clusters and Git repositories
        cleanup[variant + ':' + channel] = {
          sh "./dcos-launch delete -i info-${job_name}.json"

          sshagent(['4ff09dce-407b-41d3-847a-9e6609dd91b8']) {
            sh "git push -u origin :ci-${job_name}"
          }
        }
      }
    }
